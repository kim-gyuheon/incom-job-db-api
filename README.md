...
- 82축 코사인 유사도는 의미상 완벽히 들어맞지 않는 직무도 상위에 올라올 수 있다(축
  방향이 겹치는 다른 분야 직무가 섞여 나오는 경우) — 이 매칭 방식 자체의 특성이라
  데이터를 더 채운다고 해결되지 않는다.
- `WHISPER_MODEL_SIZE` 기본값을 `small`에서 `base`로 낮췄다 — 2026-08-26 실측으로
  small의 ct2 가중치가 462MB(fp16, int8 로드 시 ~230MB)라 Render 무료 플랜(512MB)
  안에서 FastAPI/uvicorn/sqlite 기본 오버헤드까지 더하면 여유가 거의 없었다. base는
  139MB(int8 ~70MB)로 안전 마진이 훨씬 크다 — 대신 인식 정확도는 small보다 낮다.
  그래도 OOM이 나면 `tiny`로 더 낮추거나 `openai`로 전환을 검토한다.

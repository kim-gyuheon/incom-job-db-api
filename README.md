...
프런트엔드 보안 리뷰(`frontend-security-review-corrected.md`, 원본 AI 스캔을 중복
제거·정정한 버전)에서 나온 가장 심각한(Medium)
finding에 대한 백엔드 측 대응이다: **"시작 화면으로 돌아가도 이전 사용자의 상담 세션·
전사문이 그대로 남는다"**는 문제는 근본적으로 프런트(React state 초기화 누락) 쪽에서
고쳐야 하지만, 백엔드에도 두 가지 구멍이 있었다.
